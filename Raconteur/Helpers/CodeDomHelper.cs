...
namespace Raconteur.Helpers
{
    ...
    public class CodeDomHelper
    {
        ...
        public CodeTypeReference CreateNestedTypeReference(CodeTypeDeclaration baseTypeDeclaration,
            string nestedTypeName) { return new CodeTypeReference(baseTypeDeclaration.Name + "." + nestedTypeName); }

        public void SetTypeReferenceAsInterface(CodeTypeReference typeReference)
        {
            // this hack is necessary for VB.NET code generation

            if (TargetLanguage != GenerationTargetLanguage.VB) return;

            var isInterfaceField = typeReference.GetType().GetField("isInterface",
                BindingFlags.Instance | BindingFlags.NonPublic);
            if (isInterfaceField == null) throw new InvalidOperationException("CodeDom version does not support VB.NET generation.");
        ...
        {
            var codeDomHelperRequired = target as ICodeDomHelperRequired;
            if (codeDomHelperRequired != null) codeDomHelperRequired.CodeDomHelper = this;
        }
        ...
        public CodeTypeDeclaration CreateGeneratedTypeDeclaration(string ClassName)
        {
            ...
            Result.CustomAttributes.Add(
                new CodeAttributeDeclaration(new CodeTypeReference(typeof (GeneratedCodeAttribute)),
                    new CodeAttributeArgument(new CodePrimitiveExpression("Raconteur")),
                    new CodeAttributeArgument(new CodePrimitiveExpression(CurrentVersion.ToString()))));
